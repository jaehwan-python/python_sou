# 2. 반복문 while 조건 : 조건(if)이 참인 동안 블럭을 수행한다.  가장 큰 특징 1. 초기치와 증가치가 있다. 2. while뒤에 있는 조건이 거짓이 될때까지 
#                       들여쓰기 된 부분을 수행! ..이걸 증가치가 자제해준다.

a = 1 # 조건에 초기치를 준 것. while문 이전에는 초기값을 설정해주는 작업이 필요!!
while a <= 5: # 초기 a값으로 1을 받음. a는 5보다 작거나 같다. 조건이 참!
    print(a, end = ' ') 
    a = a + 1 # 이게 없으면 위에서 수행을 a가 5보다 작거나 같을 때까지 계속 수행하지 못한다. = while문에서 못나온다. # 조건에 증가치를 준 것 
#               = while문을 탈출하게 함.
    # 참인 동안 블럭을 계속 수행한다.
...
# int('1') = 숫자 1
# str(1) = 문자 1
# i는 처음에 1이다. 참이다. 다음 while문으로 온다. j는 1이다. j = j + 1을 통해 참이 끝날 때까지 반복한다. i = 1일때 j에 대한 값이 충족될 때까지 
# 조건이 참이 되면 그 다음 i를 만족시켜준다. i = i + 1을 통해 참이도리 때까지 반복한다!!
...
while num < len(colors): # num이 colors의 리스트의 갯수보다 작다면...
        print(colors[num]) # colors[num]을 입력해라(해당 번째의 colors변수의 리스트 순서에 해당하는 값을 꺼내라). # colors[0]이라고 적어도 되지만 
#                            그러면 0으로 계속 값이 고정되어 colors[]괄호 속에 숫자가 변할 수가 없다.
