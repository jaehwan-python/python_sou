...
def listHap(*ar):
    print(ar)
    if __name__ == '__main__' :    ... 수행안됨 ... 메인모델에서 수행한 것이 아니기 때문 ... listhap은 pack1에 있다!
        print('나는 메인 모듈이야.')                 = 현재 파일의 실행상태가 / 직접 실행된 상태인가? : 파이썬 파일이 직접 실행되고 있는지, 다른 파일에서 import된 것인지 구분할 때 사용하는 특별한 변수 
'''
...
print('\n -----------------------경로 지정 방법2 : from 모듈명 import 모듈방법, ...---------------------------')
...
from pack1_other import mymod3
print(mymod3.Gop(4,5)) # .을 찍으면 이젠 그곳에 함수도 나온다. mymod3라는 파일의 Gop라는 함수에 4,5를 입력한다.
